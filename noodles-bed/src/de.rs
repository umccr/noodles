...
use serde::{de, Deserialize, Deserializer, forward_to_deserialize_any};
...
pub struct Record3Deserializer<I>
  where I: Iterator<Item = io::Result<Record<3>>> {
    records: Peekable<I>,
    state: RecordState
}
...
where I: Iterator<Item = io::Result<Record<3>>> {
  fn new(records: I) -> Self {
    Self {records: records.peekable(), state: ExpectingChrom}
  }
}
